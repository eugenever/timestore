...
unsafe fn add_value<T: NumericNative>(
    ...
    let column = (&mut (*(values_buf_ptr as *mut Vec<Vec<T>>))).get_unchecked_mut(col_idx);
        ...
        cols.iter().enumerate().for_each(|(row_idx, s)| {
            ...
            if has_nulls {
                for (col_idx, opt_v) in ca.iter().enumerate() {
                    match opt_v {
                        None => unsafe {
                            let column = (&mut (*(validity_buf_ptr as *mut Vec<Vec<bool>>)))
                ...
                for (col_idx, v) in ca.into_no_null_iter().enumerate() {
                    unsafe {
                        let column = (&mut (*(values_buf_ptr as *mut Vec<Vec<T::Native>>)))
                            .get_unchecked_mut(col_idx);
                        let el_ptr = column.as_mut_ptr();
